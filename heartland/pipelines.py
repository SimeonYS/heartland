...
class HeartlandPipeline:

    # Database setup
    conn = sqlite3.connect('heartland.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `heartland`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM heartland WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO heartland (date, title, link, content)"
